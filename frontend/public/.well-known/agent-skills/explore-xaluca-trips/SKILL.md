---
name: explore-xaluca-trips
description: Find and compare public Xaluca Tours itineraries in Morocco and their practical travel information. Use for trip discovery, not booking, payment, account access or customer records.
---
...
In supporting browsers, the site registers `xaluca_search_pages`, a read-only
WebMCP tool for finding public page titles and URLs. Otherwise use the website
and sitemap normally. This is not a remote MCP server.

For an enquiry, direct the traveller to the trip's existing contact link.
There is no public agent registration or OAuth flow. Do not use administrative
endpoints or request administrator credentials. This skill does not submit
forms, subscribe users, create reservations or make payments.
