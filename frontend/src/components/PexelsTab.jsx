...
/* Single shared fetch wrapper — accepts an AbortSignal so newer
   requests can cancel in-flight ones. */
const fetchPexels = async (path, params, signal) => {
  const qs = new URLSearchParams(params).toString();
  const r = await fetch(`${API}${path}?${qs}`, { signal });
  if (!r.ok) {
...
export default function PexelsTab({ onSelect }) {
  ...
  useEffect(() => {
    // Abort any in-flight request — guarantees responses arrive in order.
    ...
    (async () => {
      try {
        const data = debounced
          ? await fetchPexels("/api/pexels/search",  { query: debounced, page: 1, per_page: PER_PAGE }, ctrl.signal)
          : await fetchPexels("/api/pexels/curated", { page: 1, per_page: PER_PAGE }, ctrl.signal);
        if (ctrl.signal.aborted) return;
        setPhotos(data.photos || []);
        setHasMore(!!data.next_page);
      } catch (e) {
        if (e.name === "AbortError") return;       // expected when a newer query supersedes
