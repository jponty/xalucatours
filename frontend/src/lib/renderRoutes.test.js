...
test("every generated public HTML page has an exact rewrite ahead of the SPA fallback", () => {
  const paths = new Set([
  ...
  const exact = rules.filter((rule) => rule.type === "rewrite" && rule.source !== "/*");
  expect(exact).toHaveLength(paths.size);
  expect(new Set(rules.map((rule) => rule.source)).size).toBe(rules.length);
  for (const url of paths) {
    expect(exact).toContainEqual({ type: "rewrite", source: url, destination: `${url}/index.html` });
  }
  expect(exact.some(({ source }) => /[*:]|\/admin|\/api\//.test(source))).toBe(false);
});
