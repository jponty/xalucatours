...
- **Panel de Leads en /admin (Jun 2026)**: nueva pestaña "Leads" (`admin-tab-leads`) que lista los envíos de Descargar programa. `GET /api/program-downloads` ahora protegido con token admin (401 sin token). Componente `LeadsPanel` en `AdminPage.jsx`: tabla full-width (fecha, nombre, email, teléfono, programa, newsletter, idioma) + búsqueda (`admin-leads-search`), recargar (`admin-leads-refresh`) y **Exportar CSV** (`admin-leads-export`, BOM UTF-8). Verificado en navegador: 2 registros mostrados, export operativo.
