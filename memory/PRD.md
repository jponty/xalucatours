...
- **Canal de contacto preferido + "¿Qué sucede después?" (jun 2026) — COMPLETADO**: En PlannerForm y ContactForm se añadió un bloque informativo "¿Qué sucede después?" antes del botón de envío y un campo obligatorio de selección única "¿Cómo prefieres que te contactemos?" (📞 phone / ✉️ email). Componente compartido `FormExtras.jsx` (WhatHappensNext + ContactPreference, tono light/dark). Backend `preferred_contact` en ContactRequest/TripPlanner (+ email "Canal preferido"). Admin: columna "Canal pref." en ambas tablas de leads. Verificado e2e (testing agent iteration_30) + curl.
