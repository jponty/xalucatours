...
- **Overlay = nombre del lugar en Galería del recorrido (Jun 2026)**: el texto superpuesto de cada imagen ahora muestra el NOMBRE del punto destacado (no el título por-tarjeta ni nombre de archivo). `buildRouteGalleryCells` añade `nameSlot` por lugar (`${slotBase}.name` curados → mismo slot que el Mapa del día; `poi.${poiKey}.name` derivados) y `caption = lm.name`; `TripOverview` renderiza el figcaption con `EditableText slot={c.nameSlot}`. Todas las imágenes de un mismo lugar comparten ese slot único → mismo nombre, global y sincronizado en todas las páginas. Verificado Atlas-Desierto: 27 lugares, cada uno con un único nombre.
- **Sincronización Galería del recorrido ↔ Galería del lugar (Jun 2026)**: la "Galería del recorrido" (`RouteImageGallery` en `TripOverview.jsx`, builder `buildRouteGalleryCells` en `lib/routeLandmarks.js`) reúsa los slots globales de la Galería del lugar en orden de itinerario. **Causa raíz corregida**: el Mapa del día (`DayRouteMap`) prioriza landmarks CURADOS (`DAY_LANDMARKS`, slot `landmark.{id}.gallery`, galería `LANDMARK_GALLERIES[id]`) sobre `deriveDayPlaces`; la galería del recorrido solo usaba `deriveDayPlaces` → OMITÍA todos los puntos curados (días enteros faltaban). Ahora `resolveDayLandmarks` espeja la MISMA jerarquía de tiers (Tier 0 curados → Tier 1 texto/`deriveDayPlaces` → Tier 2 waypoints → Tier 3 estancia) y `buildRouteGalleryCells` calcula el `galleryBase` igual que `LandmarkCarousel` (`landmark.{id}.gallery` o `poi.{poiKey}.gallery`). **Dedupe por id de lugar** (ignorando el prefijo `landmark.`/`poi.`): un punto repetido en varios días (o que aparece como curado en un día y derivado en otro, p. ej. `xaluca-dades`) solo aporta fotos/textos en su PRIMERA aparición. Verificado en Atlas-Desierto: 27 lugares únicos, orden = secuencia de Mapas del día, 0 omisiones, 0 colisiones de id. Cambio global → todos los viajes. pestaña "Leads" (`admin-tab-leads`) con **3 subsecciones** (config-driven `LEAD_FORMS`), cada una protegida con token admin, con búsqueda, recargar y Exportar CSV: **Descargar Programa** (`program-downloads`), **Contacto Rápido** (`contact-requests`) y **Planificación Detallada** (`trip-planner`). Tabs `admin-leads-tab-{id}`. El formulario de contacto guarda ahora `source_route_id/source_path/source_label` (origen) y la columna "Origen" lo muestra. GET de contact-requests y trip-planner protegidos con token (401 sin auth). ContactForm.jsx captura el origen vía `resolvePath(location.pathname)`. Confirmado que solo existen esos 3 formularios de captación en la web. Verificado backend (401 sin token, datos OK con token) y navegador (2/14/7 registros en cada subsección).
- **PENDIENTE email**: notificación automática a joan@xaluca.com vía Resend al enviar el formulario de contacto. Playbook Resend obtenido. Falta del usuario: `RESEND_API_KEY` y remitente (`SENDER_EMAIL`, dominio verificado o `onboarding@resend.dev` en pruebas). Implementación: en `POST /api/contact-requests`, tras guardar, enviar email no-bloqueante (`asyncio.to_thread(resend.Emails.send,...)`) con todos los campos + origen; si no hay key, se omite el envío sin romper el guardado.
...
- **BLOQUEO imágenes**: RESUELTO. La key de Pexels nueva funciona; se configuró en `.env` y se descargaron 3 fotos a color por POI → `lib/extraPoiImages.js` (23×3). `buildLandmark` usa esas imágenes para los POIs nuevos. Dev: `backend/_fetch_pexels.py` (necesita User-Agent en urllib o da 403).
- **Pendiente Fase 2**: COMPLETADO. Lote 2 añadido (`extraPois.js` ahora 60 POIs): Essaouira (medina, murallas, skala, puerto, playa, islas Purpurarias), Norte (M'diq, medina Asilah/Tetuán, murallas Asilah, Plaza Uta el-Hammam, Ras el-Maa, Monumentos del Alemán), Saghro/Anti-Atlas (Saghro, N'Kob, Tafraoute, Aknioun, Amskar, Tisserdimine, Rekam, Oued Remlia, Ouzina, minas M'Fis, Fezzou+oasis, pozos Jorf, Chott Merzouga/Maïder, mercado Rissani), Dadès/M'Goun (mirador Dadès, valle Dadès, gargantas M'Goun), Ouarzazate (Atlas Studios, Kasbah Taourirt, Tizi n'Tichka, kasbah Meteorite), Khenifra NP. Todos con coords+aliases+cards (ES+EN/FR) y 3 fotos Pexels a color (`extraPoiImages.js`).
- **Imágenes y textos 100% editables por CMS**: cada card usa EditableImage/EditableText con el valor Pexels/CSV como default (slot `poi.<id>.gallery.<i>`), así que el admin puede sustituir cualquiera.
- **Verificado** testing iteration_29: los POIs aparecen en los días cuyo texto los nombra (Essaouira 3/6, Norte 3/6, etc.), con cards + fotos reales, sin errores. Los no surgidos es POR DISEÑO (el texto del día no los menciona) → coherente con "exclusivamente los lugares del recorrido descrito".
- **Duplicados omitidos** (ya cubiertos): Agafay, Alto Atlas Central, Plaza Jemaa el-Fna, Jbel Saghro (alias de saghro), Cascadas de Akchour (cubierto por akchour).
- **Mejora opcional pendiente (a decidir por cliente)**: auto-mostrar sub-puntos de una ciudad cuando el día fija la ciudad madre (p.ej. mostrar medina/murallas/playa de Essaouira cuando el día visita Essaouira aunque el texto sea genérico); y/o enriquecer narrativas de días concretos.
